...
uint256 hashGenesisBlock("0x15239b4ea4f82411a47effc8a9f1ffbe91ac6e785d1d22dbd539ff20b8ee0b2b"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // RemoCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "RemoCoin Signed Message:\n";
...
{
    // RemoCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // RemoCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, remocoin - DarkGravity v2, written by Evan Duffield - evan@remocoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, remocoin - DarkGravity v3, written by Evan Duffield - evan@remocoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // RemoCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // RemoCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        assert(block.hashMerkleRoot == uint256("0x621de0ec071f835f23ba6d264c832c89df3df6a3b1793ad5a4df6ebe17202f93"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RemoCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RemoCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RemoCoinMiner(CWallet *pwallet)
{
    printf("RemoCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("remocoin-miner");
    ...
    try { loop {
        ...
        printf("Running RemoCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RemoCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RemoCoinMiner, pwallet));
}
...
/* *** BEGIN DARKSEND MAGIC - REMOCOIN **********
    Copyright 2014, Remocoin Developers 
        eduffield - evan@remocoin.io
        InternetApe - kyle@remocoin.io
*/
